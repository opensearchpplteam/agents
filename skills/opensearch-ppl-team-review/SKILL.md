...
git fetch origin
git worktree add ../sql-pr-<pr-number>-review origin/<baseRefName>
...
Use **gh CLI only** for all GitHub interactions. Do NOT use MCP tools or
web_fetch for PR data — they consume excessive context and are less reliable
than gh.

Source order:

1. gh CLI for PR metadata, files, diff, checks, and comments
...
Spawn **at least three** `general-purpose` teammates as the baseline team.
The lead may add more teammates based on PR scope (e.g., a `docs` reviewer
for documentation-heavy PRs, a `calcite-rules` specialist for complex planner
changes, or additional domain experts as needed).

**Baseline team (always spawned):**
...
**Optional teammates (lead decides based on PR scope):**

| Teammate Name        | When to Add                             |
| :------------------- | :-------------------------------------- |
| `docs`               | PR touches user-facing docs, doctest, or REST API surface |
| `calcite-rules`      | PR modifies Calcite planner rules or optimization logic |
| `backward-compat`    | PR changes public APIs, serialization, or config defaults |

The lead should evaluate the PR diff during intake and decide whether any
optional teammates are needed before spawning the team.
...
- Sibling class pattern consistency: when a new class is added to an existing
  package, compare against sibling classes for shared defensive patterns (e.g.,
  NaN/Infinity handling, null guards, utility calls). See
  `references/review-patterns.md` section "Sibling Class Pattern Consistency"
...
- Sibling class pattern consistency: when a new class is added, verify it
  follows the same defensive patterns as sibling classes (e.g., NaN/Infinity
  handling, null guards, error wrapping). See `references/review-patterns.md`
  section "Sibling Class Pattern Consistency"
...
Approval decision is based on the final finding set:

- **Not ready for approval** if any `blocker` or `major` finding remains open
- **Ready for approval** when zero `blocker` and zero `major` findings exist
  (minor, nit, and question findings do not block approval)

Additional checks (may produce blocker/major findings):
...
## 10 Publish Review Comments

All `blocker`, `major`, and `question` findings are **auto-published** as PR
line comments after the final review output is produced. No user action needed.

For other severities (`minor`, `nit`), publish only when the user explicitly
requests: `publish minor 1`.

### Auto-Publish Sequence (Lead)

After producing the final review output, iterate over all blocker, major, and
question findings and publish each one using the steps below.

### Manual Publish

When the user asks to publish specific findings: `publish nit 1`
