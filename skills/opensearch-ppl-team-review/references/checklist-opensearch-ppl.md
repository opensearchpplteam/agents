...
### New Classes: Sibling Pattern Consistency

- When a new class is added to an existing package, compare it against sibling
  classes implementing the same interface or extending the same base class
- Verify the new class follows the same defensive patterns as siblings (e.g.,
  NaN/Infinity handling via `Utils.handleNanInfValue`, null guards, type checks)
- If a sibling pattern is intentionally skipped, flag as `question` for author
  to confirm the omission is deliberate
